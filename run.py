...
class DownLoad_M3U8(object):
    ...
    def download_all_ts(self):
        ...
        for i in range(len(ts_urls)):
            print(ts_urls[i])
            self.threadpool.submit(self.download_single_ts,[ts_urls[i],f'{i}.ts'])
...
def requests_get_key(lessonsID):
    ...
    if(len(res.text)<10):
        ...
        JSCode=open(os.path.abspath(os.path.dirname(__file__))+'\decode.js','r',encoding= 'utf8').read()
...
def ali_get_aliVodAuth(lessonsID):
    ...
    aliVodAuth=json.loads(base64.b64decode(res_json['aliVodAuth']).decode("UTF-8"))
    ...
    webPlayAuth=base64.b64encode(json.dumps(createData).encode("UTF-8")).decode("UTF-8")
    return webPlayAuth,vodVideoId

def download(method,WebPlayAuth,vodVideoId,file_name,mp4_name):
    if(method=='aliyun'):
        start = time.time()
        command=os.path.abspath(os.path.dirname(__file__))+'\core.exe '+method+' -p "'+WebPlayAuth+'" -v '+vodVideoId+' -o="'+file_name+'" --chanSize 10'
        print(command)
        os.system(command)
        end = time.time()
        print ('耗时:',end - start)
...
if __name__ == '__main__':
    ...
    for data in lessonsDatas:
        ...
        file_name = os.path.abspath(os.path.dirname(__file__))+'\\'+lessonName
        ...
        if(key != False):
            ...
            if(os.path.exists(full_path)):
                ...
                M3U8 = DownLoad_M3U8(m3u8_url,file_name,key)
                M3U8.run()
        else:
            if(os.path.exists(full_path)):
                continue
                ...
                download('aliyun',webPlayAuth,vodVideoId,file_name,mp4_name)